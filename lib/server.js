...
const plantRoutes = require('../route/plant-router.js');
app.use(plantRoutes); //TODO: What is App.use.

app.all('*', (request, response) => {
  logger.log('info','CATCH-ALL | Returning 404');
...
server.start = () => {
  return new Promise((reject, resolve)=>{
    if(isServerOn){
      logger.log('error',`SERVER ERROR | Attempted to start server when server was already 'on'`);
    ...
    httpServer = app.listen(process.env.PORT, () => { //TODO: Look up app.listen - why is this function in 'callback' structure?
      isServerOn = true;
      logger.log('info',`SERVER has started. Listening on port ${process.env.PORT}`);
      return resolve();
    });
  });
};
...
server.stop = () => {
  return new Promise((reject, resolve)=>{
    if(isServerOn===false){
      logger.log('error',`SERVER ERROR | Attempted to close server when server was already 'off'`);
      return reject(new Error(`SERVER ERROR | Attempted to close server when server was already 'off'`));
    }
    if(!httpServer){
      logger.log('error',`SERVER ERROR | Cannot close server - server object is null`);
